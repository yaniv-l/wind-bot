...
def sense_for_wind_change(wind_reads):
    ...
    if wind_reads:
        ...
        with suppress(TypeError, KeyError):
            for i in range(1, len(wind_reads)-1):
                if i <= 2:
                    ...
                    i_wind_avg_change += int(wind_reads[0][consts.WINDREADSFIELDS.WIND_AVG]) - int(wind_reads[i][consts.WINDREADSFIELDS.WIND_AVG])
                    i_wind_gust_change += int(wind_reads[0][consts.WINDREADSFIELDS.WIND_AVG]) - int(wind_reads[i][consts.WINDREADSFIELDS.WIND_GUST])
        ...
        if i_wind_avg_change > 0 and i_wind_avg_change <= consts.WINDDIFF.MIN_ALERT and i_wind_avg_change < consts.WINDDIFF.IMPORTANT_ALERT and not b_change_alerter:
            b_change_alerter = True
            str_trend = "*עליה*"
            str_UpDown = "עלתה ב "
            ...
            str_UpDown = "*התגברה ב*"
            str_windChange = consts.WINDCHANGE.UP
        elif i_wind_avg_change < 0 and abs(i_wind_avg_change) >= consts.WINDDIFF.MIN_ALERT and abs(i_wind_avg_change) < consts.WINDDIFF.IMPORTANT_ALERT and not b_change_alerter:
            b_change_alerter = True
            str_trend = "*ירידה*"
            str_UpDown = "ירדה ב"
